...
from isaacsim.core.experimental.objects import Camera, Cube, Cylinder, DistantLight, GroundPlane  # noqa: E402
...
drone_prims = {i: make_quad(f"/World/drone_{i}", start, 0.0) for i, start in enumerate(SCENE.drone_starts)}

# --- Cameras -----------------------------------------------------------------
NADIR_QUAT = [0.0, 1.0, 0.0, 0.0]  # USD cameras look down -Z; flip so -Z points at the ground with +x forward
...
if args.score != "none":
    ...
    for i, start in enumerate(SCENE.drone_starts):
        path = f"/World/nadir_{i}"
        cam = Camera(paths=path, positions=[list(start)], orientations=[NADIR_QUAT])
...
while simulation_app.is_running() and t < args.duration:
    ...
    if timeline.is_playing():
        for i, prim in drone_prims.items():
            if i in latest_poses:
                ...
                if i in nadir:
                    nadir[i][0].set_world_poses(positions=[pos], orientations=[NADIR_QUAT])
        if latest_poses:
    ...
    simulation_app.update()
    if timeline.is_playing() and scorer is not None and latest_poses and t >= next_score:
